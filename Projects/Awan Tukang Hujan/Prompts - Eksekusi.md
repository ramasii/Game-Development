...
> Update: planet 40 slot (34 soil + 6 water).
...
```
 ...
 Batasan keras GDD: Mobile Landscape 2D, 1 jari drag saja, no angka/huruf di gameplay & UI (full ikon), no win, no HUD bunga, lose tunggal AliveCount==0, planet 40 slot (34 soil + 6 water, tiap slot 1 sprite Opsi A), tanaman initial 17 maks 34, max 5 awan, Unity 6 + URP 2D.
 Arsitektur wajib: SSOT BalanceConfigSO (termasuk soilCount=34, waterCount=6, initialPlantCount=17), GameManager Singleton + GameState enum (Boot,MainMenu,Playing,Paused,GameOver), State Pattern untuk Cloud/Soil/Plant, Observer EventChannelSO, Factory + Object Pool, MVP UI, SOLID, Runtime State Separation, Dirty Flag (wajib untuk 34 soil), Flyweight.
 Larangan: JANGAN tambah mekanik/sistem di luar GDD. JANGAN hardcode timer/slot count di script. JANGAN buat scene baru selain Game. JANGAN drag manual 40 slot satu-satu (wajib loop spawn dari data). Jika ragu, baca vault dulu.
```
...
DoD: folder + asmdef lengkap, GameManager bisa SetState via Inspector button/context, BalanceConfig terbaca semua sistem (tidak ada angka hardcode, termasuk 34/6/17), kompilasi bersih.
JANGAN lanjut ke planet/awan. Hanya fondasi.
```

## P1 - PlanetRoot + 40 Slot Whitebox (34 Soil + 6 Water) + Rotasi Drag

```
Konteks: baca ATURAN GLOBAL + GDD bab 3 Mekanik 4+5 + Planning bab 2 via MCP.

Tugas: dunia melingkar 40 slot bisa diputar (Unity MCP only).
...
2. Buat SoilSlot.prefab whitebox (SpriteRenderer kotak, beda warna Dry pucat vs Wet gelap) + SoilStateMachine.cs (State Pattern: Dry <-> Wet, timer soilWetDuration, rainToWet 2 dtk untuk jadi Wet, Dirty Flag untuk visual — wajib karena 34 soil). Spawn 40 slot melingkar via loop data sebagai child PlanetRoot (34 soil + 6 water selang-seling sesuai planetLayout di BalanceConfig, radius konsisten, terlihat menyatu). JANGAN drag manual 40x.
3. Buat WaterSpot.cs x6 (child PlanetRoot, zona evaporasi + visual biru) + EvaporasiZone detection (stay 1 dtk = tick).
...
5. Verifikasi: Play mode, drag tanah muter 360° balik ke awal, kamera diam, 40 slot ikut muter. Cek via unity_graphics_scene_capture jika perlu. Cek FPS tidak drop.

DoD: 40 slot (34 soil + 6 water) muter mulus, tidak mabuk (clamp+easing), soil bisa Wet/Dry via debug hujan dummy, tidak ada awan dulu.
```
...
4. Evaporasi: stay di atas WaterSpot (salah satu dari 6) 1 dtk → growth tick + uap placeholder (lingkaran naik) → Small bisa jadi Normal.
5. Hujan placeholder: Dark auto-rain (box biru jatuh via Debug particle/line) → kirim OnRainTick ke SoilSlot di bawahnya (worldX → slot index terdekat dari 34 soil) → soil jadi Wet. Hujan sampai menyusut jadi Small.
...
Tugas: ekosistem hidup/mati + lose tunggal (40 slot scale).

1. Finalisasi SoilSlot x34: Dry/Wet penuh + Puddle child (kotak biru muda fade in/out, Dirty Flag). rainToWet 2 dtk, wet tahan 25 dtk.
2. Buat Plant.prefab (whitebox tunas: scale + warna) + PlantStateMachine (Seed -> Growing jika Wet terus 15 dtk -> Thirsty jika kering 10 dtk -> Dead jika kering lanjut 12 dtk / BloomVisual jika subur 20 dtk, bunga = visual only bukan win). Parent ke SoilSlot (17 tanaman initial, maks 34, ikut muter, jumlah via BalanceConfig initialPlantCount).
...
4. Anti-softlock: jika awan 0 tapi Alive>0 → force spawn 1 Normal 2 dtk dari genangan terdekat (dari 6 water).
5. Verifikasi Unity MCP: bunuh semua tanaman via test (keringkan) → GameOver kepicu. Siram rutin → bertahan.

DoD: loop merge-hujan-tumbuh-mati jalan di 40 slot, GameOver hanya saat tanaman habis, bukan saat awan habis. Event decoupled, tidak ada reference langsung Plant→UI.
```
...
Tugas: rapikan spawn + juice tanpa ubah gameplay (skala 40 slot).

1. Buat CloudFactory.cs (IProduct + Initialize dari VisualSO) + PoolManager (UnityEngine.Pool ObjectPool<Cloud>, RainDrop, Puddle, Uap — size sesuaikan 40 slot). Ganti semua Instantiate/Destroy ke Pool.Get/Release.
2. Ganti whitebox: awan jadi sprite blob lucu (mata 2 titik, no teks), rain = particle streak biru, uap = puff putih, puddle = blob pipih. Warna via VisualSO (Flyweight). Tetap Unlit 2D, sorting Sky > Planet. Siapkan sprite atlas untuk 40 slot + tanaman biar draw call aman.
...
Tugas: navigasi planet 40 slot + state flow lengkap (ikon only).

1. EdgeScrollDetector: jika awan di-drag ke 15% edge kanan/kiri → PlanetRotator auto-rotate proporsional + tampilkan panah edge (sprite segitiga fade). Deadzone jelas biar tidak kepencet tidak sengaja. Drag tanah tetap manual (penting untuk 40 slot yang butuh muter jauh).
...
4. Restart reset Run saja (40 slot reset: 34 soil kering ulang + tanaman hidup ulang initialPlantCount, 2 Normal). Home lerp rotasi ke 0.
...
DoD: pemain baru tanpa teks paham merge-hujan-muter 40 slot dalam 5 menit. Mute + FTUE flag persist. Tidak ada narasi teks/suara berhuruf.
```

## P7 - Balancing Survival 40 Slot + Build APK + Validasi Go/No-Go
...
Tugas: kunci fun endless 40 slot + buktikan lolos.

1. Buat spreadsheet balancing (atau tabel di Planning): uji 3 set (gampang v0.1, sedang, susah) untuk soilWet 25, death 12, cloudLife 8/40 dengan 34 soil / 6 water / 17 tanaman. Target: bertahan 3-8 menit ideal. Mati <1 menit = terlalu susah, >10 menit bosen = terlalu gampang. Perhatikan rasio 6 water : 34 soil (cari air = tantangan, air rebutan). Tuning hanya via BalanceConfigSO asset, JANGAN ubah kode.
2. Via Unity MCP: switch platform Android, cek Player Settings landscape, package name, icon awan. Cek perf 40 slot (draw call atlas, dirty flag aktif). Build APK via unity_build target Android. Catat error jika ada, fix via unity_get_compilation_errors.
3. Playtest buta 5 menit (atau minta user test): checklist Go/No-Go:
 - Bisa merge + hujan + muter 40 slot tanpa teks? ya/tidak
 - Bertahan >3 menit + mau retry? ya/tidak
 - Edge salah trigger >3x? harus tidak
 - Merge terasa random? harus tidak
 - FPS drop karena slot? harus tidak
