...
- **Design Pattern Pilihan:**
  - `SSOT` — `BalanceConfigSO` + `Cloud/Soil/PlantVisualSO` (Flyweight). Tidak ada hardcode timer di script.
  ...
  - `Factory + Object Pool` — `CloudFactory` + `UnityEngine.Pool` untuk Cloud/Rain/Uap/Puddle. Anti GC spike mobile.
  - `MVP` — UI ikon-only: MainMenu / Pause / GameOver / HUD-PauseOnly. View animasi, Presenter dengar event.
  - `Dirty Flag` — SoilSlot update warna cuma pas berubah.
  - `Singleton hemat` — hanya GameManager, PoolManager, AudioManager, PlantManager (AliveTracker).
- **Arsitektur & Penyimpanan Data:** ScriptableObject-based SSOT untuk balance + visual + event channel. Runtime split: Persistent (FTUE flag PlayerPrefs) vs Run (soil wet timer, plant state, cloud tier, planet angle). Nanti save cukup PlayerPrefs untuk FTUE + best survival (waktu, tanpa tampilkan angka ke pemain? simpan internal saja).
- **Mermaid Diagram:**
    ```mermaid
    graph TD
...
```
- **Rencana Optimisasi:**
  - *CPU/Memori:* Object Pool semua spawn berulang, Dirty Flag untuk soil, clamp + easing rotasi planet.
  - *Rendering/UI:* 1 scene, kamera ortho fixed, Canvas split statis/dinamis nanti, sprite Unlit + SpriteMask untuk busur planet.
...
- **Risiko Teknis:** Merge overlap terasa adil (perlu magnet snap + hold 0.4 dtk), edge-scroll kepencet tidak sengaja (perlu deadzone + indikator), rotasi planet bikin mabuk (clamp 60 deg/dtk + easing).
- **Risiko Desain:** Core bertahan >3 menit harus fun tanpa teks. Validasi via playtest buta 5 menit.
